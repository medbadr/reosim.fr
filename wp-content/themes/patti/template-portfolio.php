			...
			<ul class="portfolio <?php echo $layout; ?> isotope grid_portfolio">
				...
				if(!empty($lgal)) {
					foreach($lgal as $gal_item) {
						...
						$gal_output .= '<a class="hidden_image" href="'.esc_url($gal_item_url).'" rel="prettyPhoto[gallery_'.$post->ID.']" title="'.esc_attr($gal_item_title).'"></a>';
				...
				<li class="grid-item <?php if($terms) { foreach ($terms as $term) { echo get_taxonomy_cat_ID($term->name) .' '; } } else { echo 'none '; } ?><?php if($layout == 'grid') { echo $item_class; } ?>">
					...
					if($layout == 'grid') {
						...
						<?php } 
							...
							<a href='<?php echo esc_url($portf_link); ?>'>
						<?php }	
						else if ($portf_icon == 'lightbox_to_video') {  ?>
							<a rel="prettyPhoto[portfolio_gallery]" href="<?php echo esc_url($portf_video); ?>" title="<?php the_title(); ?>">
						<?php }	
						...
						?>
							...
							<img src="<?php echo esc_url($grid_thumbnail); ?>" alt="" />
						</a>	
